...
export async function storeTransactionNode(
  ...
  const workflowExecutionId =
    ...
    generateFallbackWorkflowExecutionId(state.telegramUserId);
  ...
  const transactionInput: TransactionInput = {
    ...
    image_data_hash: state.imageData
      ? generateImageHash(state.imageData)
      : undefined,
  ...
  return {
    error: `Failed to store transaction after ${maxRetries + 1} attempts: ${
